...
export default function NoticeBoard({ canManage = false }: Props) {
  ...
  const isAdmin = useIsAdmin(); // Check admin status directly
  const showManageTab = canManage || isAdmin; // Show if either prop is true or user is admin
  ...
  useEffect(() => {
    if (!showManageTab && tab === "admin") setTab("student");
  }, [showManageTab, tab]);
  ...
  return (
    <div className="max-w-6xl mx-auto w-full px-4 md:px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex rounded-xl overflow-hidden border-2 shadow-md">
          <button
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {filtered.map((n) => (
                <NoticeCard
                  key={n.id}
                  notice={n}
                  onEdit={showManageTab ? setEditing : undefined}
                  onDelete={showManageTab ? handleDelete : undefined}
                />
              ))}
            </div>
          ) : (
